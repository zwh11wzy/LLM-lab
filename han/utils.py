...
def setup_log_dir(args, sampling=False):
    ...
    log_dir = os.path.join(args["log_dir"], "{}_{}".format(args["dataset"], date_postfix))
...
def load_acm_raw(remove_self_loop):
    ...
    p_vs_c_filter = p_vs_c.tocsr()[:, conf_ids]
    p_selected = (p_vs_c_filter.sum(1) != 0).A1.nonzero()[0]
    p_vs_l = p_vs_l.toarray()[p_selected]
    p_vs_a = p_vs_a.toarray()[p_selected]
    p_vs_t = p_vs_t.toarray()[p_selected]
    p_vs_c = p_vs_c.toarray()[p_selected]
    ...
    features = torch.FloatTensor(p_vs_t)
